...
| **search** | `query`, `collection` (opt), `n` (opt, default 5), `threshold` (opt, default 0.0) | ranked results with scores | "What did I save about distributed Erlang?" |
...
- **Retrieve relevant context:** `search` by meaning — no keyword matching, returns semantically similar passages with a 0–1 relevance score
